...
class IshiharaEndpoints(viewsets.ViewSet):
    
    def list(self, request, *args, **kwargs): 
        all_test =  LangIshiharaTest.objects.all()
        lang_test = all_test.filter(type_test=TypeClassicTest.ISHIHARA.value)
    ...
    def create(self, request, *args, **kwargs):
        """
        Create new card of the Test ishihara
        """
        lang_test_serializer = LangIshiharaSerializer(data = request.data)
        lang_test_serializer.is_valid(raise_exception=True)
        lang_test_serializer.context["type_test_default"] = TypeClassicTest.ISHIHARA.value
