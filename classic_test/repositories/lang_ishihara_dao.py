...
class LangIshiharaDao():
    ...
    @staticmethod
    def find_test_by_id_and_type(id, type_test):
        try:
            test = LangIshiharaTest.objects.all().get(id=id, type_test=type_test)
            if test is None:
                raise NotFoundException(message_english=f'Test not found by id = {id} and type test = {type_test}',
                                        message_spanish=f'Prueba no encontrada, con el id = {id} and type test = {type_test}',
                                        code=TypeAppError.TEST_LANG_ISHIHARA_NOT_FOUND.value)
            return test
        except ObjectDoesNotExist:
            raise NotFoundException(message_english=f'Test not found by id = {id} and type test = {type_test}',
